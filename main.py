...
class VLCSync:
    ...
    def force_sync(self):
        ...
        if self.is_host and self.player.is_playing():
            ...
            print("Forced synchronization.")
        elif not self.is_host:
            print("Only the host can force synchronization.")

    def handle_sync(self, command):
        """Handle sync command from host with exact time synchronization."""
        if not self.is_host and self.player.is_playing():
            host_time = command["time"]
            self.player.set_time(host_time)

    def sync_playback(self):
        """Synchronize playback exactly every 3 seconds."""
